...
SKIN.PrintName 		= "PAC Server Skin"
SKIN.Author 		= "Prefanatic"
SKIN.DermaVersion	= 1

SKIN.bg_color 					= Color( 0, 0, 0, 150 )
SKIN.bg_color_sleep 			= Color( 70, 70, 70, 255 )
SKIN.bg_color_dark				= Color( 50, 50, 50, 100 )
SKIN.bg_color_bright			= Color( 220, 220, 220, 255 )

SKIN.fontFrame					= "Trebuchet22"
...
SKIN.colPropertySheet 			= Color( 50, 50, 50, 50 )
SKIN.colTab			 			= Color( 80, 80, 130, 100 )
SKIN.colTabInactive				= Color( 10, 10, 50, 50 )
...
SKIN.colMenuBorder				= Color( 0, 129, 211, 200 )
SKIN.colMenuClose				= Color( 200, 100, 100, 150 )
SKIN.colMenuClose_Shadow		= Color( 150, 50, 50, 150 )

SKIN.colButtonText				= Color( 255, 255, 255, 255 )
SKIN.colButtonTextDisabled		= Color( 255, 255, 255, 55 )
SKIN.colButtonFill				= Color( 0, 0, 0, 100 )
SKIN.colButtonHighlight			= Color( 80, 80, 150, 100 )
SKIN.colButtonDisabled			= Color( 0, 0, 0, 50 )
SKIN.colButtonBorder			= Color( 0, 129, 211, 200 )
SKIN.fontButton					= "Trebuchet18"

SKIN.colPanelList				= Color( 50, 50, 50, 100 )
...
function SKIN:SplitColor( col )

	return col.r, col.g, col.b, col.a
	
end

function SKIN:DrawOutlinedBox( x, y, w, h, color )

	surface.SetDrawColor( self:SplitColor( color ) )
	surface.DrawRect( x, y, w, h )
	
	surface.SetDrawColor( self:SplitColor( self.colMenuBorder ) )
...
function SKIN:PaintFrame( panel )
	...
	self:DrawOutlinedBox( 0, 22, panel:GetWide(), panel:GetTall() - 22, color ) -- Draw fill and inner outline.
	
	self:DrawOutlinedBox( 0, 0, panel:GetWide(), 22, panel.BFill or Color( 100, 100, 200, 100 ) ) -- Top Fill
	 
end

function SKIN:DrawFrameClose( panel )

	local button = panel.btnClose

	button:SetPos( panel:GetWide() - 45, 3 )
	button:SetSize( 40, 15 )
	
	button.Paint = function()

		local split = button:GetTall() * ( 2 / 3 )
			
		surface.SetDrawColor( self:SplitColor( self.colMenuClose ) )
		surface.DrawRect( 0, 0, button:GetWide(), split )
		
		surface.SetDrawColor( self:SplitColor( self.colMenuClose_Shadow ) )
		surface.DrawRect( 0, split, button:GetWide(), button:GetTall() - split )
...
function SKIN:LayoutFrame( panel )
	...
	self:DrawFrameClose( panel )
	
	panel.lblTitle:SetPos( 8, 1 )
	panel.lblTitle:SetSize( panel:GetWide() - 45, 20 )
...
function SKIN:PaintButton( panel )
	...
	if ( panel.m_bBackground ) then
		...
		local col = self.colButtonFill
		
		if ( panel:GetDisabled() ) then
			col = self.colButtonDisabled
		elseif ( panel.Depressed || panel:GetSelected() ) then
			col = self.colButtonHighlight
...
function SKIN:PaintPanelList( panel )
	...
	if ( panel.m_bBackground ) then
		draw.RoundedBox( 4, 0, 0, panel:GetWide(), panel:GetTall(), self.colPanelList )
	end
...
derma.DefineSkin( "PACSkin", "It looks cool bro.", SKIN )
